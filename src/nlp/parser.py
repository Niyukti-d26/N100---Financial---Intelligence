...
def parse_metric(company_id, metric_name, text):
    ...
    match = re.search(r"(\d+)\s*Years?:?\s*(-?[\d.]+)%", text)
    ...
    ttm_match = re.search(r"TTM:?\s*(-?[\d.]+)%", text)
    ...
    last_year_match = re.search(r"Last Year:?\s*(-?[\d.]+)%", text)
...
def run_parser():
    ...
    for _, row in analysis.iterrows():
        ...
        for metric in metric_columns:
            ...
            result = parse_metric(company_id, metric, row[metric])

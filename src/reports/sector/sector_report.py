...
def generate_sector_reports():
    """Function: generate_sector_reports"""
    ...
    master = latest_ratios.merge(
        sector_map, left_on="company_id", right_on="company_id", how="left"
    ...
    for sector_name in sectors["broad_sector"]:
        ...
        sector_df = master[master["broad_sector"] == sector_name].copy()

        pdf_path = f"reports/sector/" f"{sector_name}_report.pdf"

        doc = SimpleDocTemplate(pdf_path)

        elements = []

        elements.append(Paragraph(sector_name, styles["Title"]))

        elements.append(Paragraph(f"Companies: {len(sector_df)}", styles["Normal"]))
        ...
        median_npm = sector_df["net_profit_margin_pct"].median()

        median_de = sector_df["debt_to_equity"].median()

        median_fcf = sector_df["free_cash_flow_cr"].median()

        median_rev_cagr = sector_df["revenue_cagr_5yr"].median()
        ...
        elements.append(Paragraph("Companies", styles["Heading1"]))

        company_table = [
            ["Company", "ROE", "ROCE", "NPM", "Debt/Equity", "FCF", "Revenue CAGR"]
        ]

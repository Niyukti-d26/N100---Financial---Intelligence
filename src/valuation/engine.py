...
class ValuationEngine:
    ...
    def generate_valuation(self):
        ...
        market = market[market["year"] == latest_year]

        cashflow = cashflow[cashflow["year"] == latest_year]

        # Remove duplicate company rows
        market = market.drop_duplicates(subset=["company_id"])

        cashflow = cashflow.drop_duplicates(subset=["company_id"])

        df = market.merge(cashflow, on=["company_id", "year"], how="left")
        ...
        df["valuation_label"] = df["pe_ratio"].apply(pe_flag)
